    ...
    async function loadOverview() {
        try {
            ...
            if (stats) {
                ...
                document.querySelector('.stat-card:nth-child(3) .value').textContent = stats.todayOrders || 0;
            }

            if (balance) {
                // Update admin wallet balance
                const walletEl = document.getElementById('adminWalletBalance');
                if (walletEl && balance.totalRevenue) {
                    walletEl.textContent = `₹${balance.totalRevenue.toLocaleString()}`;
                }
            }
    ...
    async function loadMenu() {
        try {
            ...
            if (!menu || menu.length === 0) {
                menuGrid.innerHTML = '<div class="empty-state">No menu items found. Add your first item!</div>';
                return;
            }
            ...
            menu.forEach(item => {
                ...
                const emoji = item.category === 'Burgers' ? '🍔' :
                ...
                card.innerHTML = `
                    ...
                    <div class="menu-details">
                        <h4>${item.name}</h4>
                        <p>₹${item.price}</p>
                        <span style="font-size: 11px; color: var(--text-muted);">${item.category || 'Uncategorized'}</span>
    ...
    window.promptEditMenu = async (id) => {
        ...
        if (newPrice && !isNaN(newPrice)) {
            try {
                ...
                showNotification(`Price updated for ${item.name}`, 'success');
                loadMenu();
            } catch (err) {
    ...
    async function loadGiftCards() {
        try {
            ...
            if (!registry || registry.length === 0) {
                grid.innerHTML = '<div class="empty-state">No gift cards in registry</div>';
                return;
            }
            ...
            registry.forEach(card => {
                ...
                const redeemedDate = card.redeemedAt ? new Date(card.redeemedAt).toLocaleDateString() : 'N/A';
                item.innerHTML = `
                    <div class="gc-info">
                        <strong>Code: ${card.code}</strong>
                        <span>Value: ₹${card.amount} (+₹${card.bonus} bonus)</span>
                        <span>Target: ${card.targetUserId}</span>
                        <span>Purchased by: ${card.purchasedBy}</span>
                        ${card.isRedeemed ? `<span>Redeemed: ${redeemedDate}</span>` : ''}
            ...
            const grid = document.getElementById('giftCardRegistry');
            grid.innerHTML = '<div class="empty-state" style="color: var(--error);">Failed to load gift card registry</div>';
        }
    }
